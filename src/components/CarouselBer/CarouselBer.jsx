...
function CarouselBer() {
    ...
    const TOTAL_ORIGINAL = 10; // Количество реальных карточек
    const VISIBLE = 5;
    const CARD_WIDTH = 255 + 16; // Ширина одной карточки с отступом

    // Создаем базовый массив продуктов (только 10 реальных элементов)
    ...
    const handleNext = () => {
        setCurrentIndex((prev) => (prev + 1) % TOTAL_ORIGINAL); // Цикл: 0->1->...->9->0
    };

    useEffect(() => {
        if (milkStuffRef.current) {
            const maxTranslate = (TOTAL_ORIGINAL - VISIBLE) * CARD_WIDTH; // Максимум 5 шагов
                ...
                <div className="milk-stuff" ref={milkStuffRef}>
                    {products.map((product, index) => (
                        <div className="product-container" key={index}>
                            ...
                            <p className="price">{product.price}₽<span className="oldPrice">{product.oldPrice}₽</span></p>
